...
def salary_dashboard(input_df):
    ...
    selected_company = multiselect_from_dataframe("Company", input_df, "ragione_sociale_azienda", sidebar=True, default=["Relatech Spa"])
    ...
    metrics = {
        ...
        "remaining_holidays": round(dynamic_dataframe['totale_ferie_rimanenti'].iloc[-1], 2) if not dynamic_dataframe.empty else 0,
        "remaining_permissions": round(dynamic_dataframe['totale_permessi_rimanenti'].iloc[-1], 2) if not dynamic_dataframe.empty else 0
    }
    ...
    for i, (label, key) in enumerate(metrics_labels):
        cols[i % 3].metric(label, f"{formatted_metrics[key]} €" if "€" in label else formatted_metrics[key])

    # Net salary time series chart
    static_dataframe['month'] = static_dataframe['date_periodo_di_retribuzione'].dt.to_period('M').astype(str)
    df_aggregated = static_dataframe.groupby('month', as_index=False)['netto_del_mese'].sum()
    ...
    # Filtered data table
    dynamic_dataframe = (dynamic_dataframe
    ...
    dynamic_dataframe.columns = [col.replace('_', ' ').title() for col in dynamic_dataframe.columns]
