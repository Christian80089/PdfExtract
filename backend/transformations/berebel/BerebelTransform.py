...
from backend.resources.constants.berebel.BerebelConstants import berebel_df_mandatory_fields, berebel_df_schema
...
def transform_df(df, columns_to_select):
    ...
    for field in berebel_df_mandatory_fields:
    ...
    selected_df = cast_columns_with_defaults(selected_df, berebel_df_schema, df_default_values)

    # Arrotonda i valori float per eccesso a due cifre decimali
    for column, dtype in berebel_df_schema.items():
        if dtype == "float":
